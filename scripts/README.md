...
| `proxmox-ol9-test-vm.sh` | Local-only Proxmox utility for a disposable OL9 Spinifex test VM. Requires a scoped API token in `~/.config/spinifex/proxmox.env`, an already imported OL9 QCOW2 volume, and a 1-8 character `PVE_ROOT_PASSWORD` supplied only in the environment. Uses `cpu=host`, a VirtIO boot disk, 2 vCPUs/2 GiB by default, standard VGA, and an expiring root password. The short password is test-only and must never be reused. `PVE_ENABLE_ROOT_SSH_PASSWORD_LOGIN=1` is an explicit local-test-only escape hatch; it enables root password SSH solely so automated tests can reach the disposable guest. `create` refuses to replace a VM; `destroy` is explicit and only accepts `spinifex-ol9-*` names. The guest installer uses only Oracle-signed OL9 and oVirt RPM repositories; it installs no private Spinifex RPM repository, EPEL repository, Ansible, or other configuration manager. |
