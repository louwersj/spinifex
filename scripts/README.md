...
| `proxmox-ol9-test-vm.sh` | Local-only Proxmox utility for a disposable OL9 Spinifex test VM. Requires a scoped API token in `~/.config/spinifex/proxmox.env`, an already imported OL9 QCOW2 volume, and a 1-8 character `PVE_ROOT_PASSWORD` supplied only in the environment. Uses `cpu=host`, a VirtIO boot disk, 2 vCPUs/2 GiB by default, standard VGA, and an expiring root password. The short password is test-only and must never be reused. `PVE_ENABLE_ROOT_SSH_PASSWORD_LOGIN=1` is an explicit local-test-only escape hatch: it enables password SSH and deliberately suppresses first-login password expiry so automated tests can reach the disposable guest. It must never be used outside the isolated test VM. `create` refuses to replace a VM; `destroy` is explicit and only accepts `spinifex-ol9-*` names. The guest installer uses only Oracle-signed OL9 and oVirt RPM repositories; it installs no private Spinifex RPM repository, EPEL repository, Ansible, or other configuration manager. |
| `install-ol9-spinifex.sh` | Fully automated, guest-side Oracle Linux 9 installer contributed by Johan Louwers. It is designed for `curl | sudo env ... bash` from a version-pinned GitHub Release: it runs the production `setup.sh`, installs the matching `ol9-amd64` release tarball, enables and starts `spinifex.target`, then verifies the target is active. It deliberately refuses other distributions and requires only Oracle-signed runtime repositories—no private RPM repository, EPEL, Ansible, repository credentials, or embedded RPMs. |
